...
const TaskModal = ({ members, onDelete, onUpdate, userRole }: TaskModalProps) => {
  ...
  const handleSave = () => {
    if (task) {
      onUpdate(task.id, { title, description, assignedUserId: assignedUserId === '' ? null : assignedUserId, status: status as Task['status'] });
      closeTaskModal();
    }
  };
  ...
  const handleUnassign = async () => {
    if (task) {
      ...
      try {
        await onUpdate(task.id, { assignedUserId: null });
        setAssignedUserId('');
        toast.success("Task unassigned successfully");
        setShowUnassignModal(false);
      } catch (error) {
        // Error toast is handled by Board.tsx
      } finally {
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-2">
                ...
                {assignedUserId && canAssignActual && (
                  <Button variant="destructive" onClick={() => setShowUnassignModal(true)} className="rounded-lg shrink-0" title="Unassign User">
                    Unassign
                  </Button>
                )}
              </div>
      ...
      <Dialog open={showUnassignModal} onOpenChange={setShowUnassignModal}>
        <DialogContent className="rounded-xl sm:max-w-md">
          ...
          <DialogFooter className="gap-2 sm:gap-0">
            <Button variant="outline" className="rounded-xl" onClick={() => setShowUnassignModal(false)}>
              Cancel
            </Button>
            <Button variant="destructive" className="rounded-xl" onClick={handleUnassign} disabled={isUnassigning}>
